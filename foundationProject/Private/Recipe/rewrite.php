...
if(isset($_POST["change"])){
    $query = $conn->prepare("update recipe set Name=:Name, Recipe=:Recipe where Id=:Id;");
    unset($_POST["change"]);
    $query->execute($_POST);
    ...
    <div class="grid-x" style="justify-content:center;">
        <div class="cell medium-4 large-3">
            <form method="post" action="<?php echo $_SERVER["PHP_SELF"];?>">
                <h4 class="text-center">Promjeni recet</h4>
                <label>Naziv
                    <input value="<?php echo $result->Name;?>" type="text"  name="Name">
                </label>
                <label>Opis
                    <input value="<?php echo $result->Calories;?>" type="number"  name="Description" >
                </label>
                <br>
